...
export class HookRunner {
  ...
  private shouldIgnore(files: string[]): boolean {
    ...
    return files.some(file =>
      this.ignorePatterns.some(pattern => this.matchIgnorePattern(file, pattern))
    )
  }
  ...
  private matchIgnorePattern(file: string, pattern: string): boolean {
    ...
    if (pattern.endsWith('/')) {
      ...
      return file.startsWith(pattern) || file.includes('/' + dirPattern + '/') || file.endsWith('/' + dirPattern)
    }
    ...
    return file === pattern || file.endsWith('/' + pattern) || file.includes('/' + pattern + '/')
  }
  ...
  async run(name: string, args: string[] = []): Promise<boolean> {
    ...
    const shouldParallel = (hook.parallel ?? this.parallel) && commands.length > 1
