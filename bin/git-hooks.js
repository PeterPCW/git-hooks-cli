 ...
 */
function resolveScript(scriptName, pkg) {
...
function showHelp() {
    ...
    const title = (text) => useColors ? colorize(text, 'cyan') : text;
    const cmd = (text) => useColors ? colorize(text, 'green') : text;
    const opt = (text) => useColors ? colorize(text, 'gray') : text;
    console.log(`
...
function installHook(hookName, silent = false) {
    const useColors = supportsColor();
    const ok = (text) => useColors ? colorize(text, 'green') : text;
...
function listHooks() {
    const useColors = supportsColor();
    const installed = (text) => useColors ? colorize(text, 'green') : text;
    const notInstalled = (text) => useColors ? colorize(text, 'gray') : text;
    const config = getConfiguredHooks();
    const gitDir = getGitDir();
    const isWindows = process.platform === 'win32';
...
function checkStatus() {
    const useColors = supportsColor();
    const ok = (text) => useColors ? colorize(text, 'green') : text;
    const warn = (text) => useColors ? colorize(text, 'yellow') : text;
    const fail = (text) => useColors ? colorize(text, 'red') : text;
    const dim = (text) => useColors ? colorize(text, 'gray') : text;
    const config = getConfiguredHooks();
    const gitDir = getGitDir();
    const isWindows = process.platform === 'win32';
...
function checkConfig() {
    ...
    if (Object.keys(config).length === 0) {
        ...
        const hasParallel = Object.values(config).some(c => typeof c === 'object' && c !== null && 'parallel' in c);
        const hasIgnore = Object.values(config).some(c => typeof c === 'object' && c !== null && 'ignore' in c);
        if (hasParallel) {
...
async function runHook(hookName, args) {
    ...
    if (Array.isArray(hookConfig)) {
        ...
        const runConfig = configObj.run;
        if (Array.isArray(runConfig)) {
            commands = runConfig.map((s) => resolveScript(s, pkg || {}));
        }
        else if (typeof runConfig === 'string') {
            commands = [runConfig];
        }
    ...
    const hookConfigObj = typeof hookConfig === 'object' && hookConfig !== null ? hookConfig : null;
